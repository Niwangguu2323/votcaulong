...
class MongoDBCauLongPipeline:
    def __init__(self):
        # Kết nối tới MongoDB
        self.client = pymongo.MongoClient('mongodb://192.168.11.164:27017')
        self.db = self.client['dbvotcaulong']  # Tạo hoặc kết nối Database
    ...
    def process_data_from_mongodb(self):
        ...
        # Bước 2: Lấy tất cả dữ liệu sau khi đã xóa
        data = collection.find()

        # Bước 3: Loại bỏ các bản ghi có cột 'ma' trùng nhau
        unique_data = []
        seen_ids = set()  # Sử dụng bộ để lưu trữ các mã sản phẩm đã gặp

        for item in data:
            product_id = item.get('ma')  # Lấy giá trị của cột 'ma'
            
            if product_id not in seen_ids:  # Nếu mã sản phẩm chưa tồn tại trong bộ
                unique_data.append(item)    # Thêm bản ghi vào danh sách không trùng lặp
                seen_ids.add(product_id)    # Thêm mã sản phẩm vào bộ

        # Bước 4: Xử lý dữ liệu null và chuẩn bị dữ liệu
        processed_data = self.process_data(unique_data)

        # Bước 5: Lưu dữ liệu đã xử lý vào PostgreSQL
    ...
    def process_data(self, data):
        ...
        for item in data:
            ...
            if 'trongLuong' in item and item['trongLuong']:
                ...
                for line_number, (weight_type, weight_range) in enumerate(weight_lines, start=1):
                    ...
                    # Thêm số dòng vào mã sản phẩm
                    new_item['ma'] = f"{new_item.get('ma')}_{line_number}"  # Thêm số dòng vào mã
    ...
    def save_to_postgres(self, data):
        ...
        for item in data:
            cursor.execute('''
                INSERT INTO caulong (ma, tensp, gia, thuongHieu, courseUrl, tinhTrang, trinhDo, noiDung, phongCach, doCung, diemCanBang, weight_type, weight_range, thongTin)
...
class CSVDBCauLongPipeline:
    def process_item(self, item, spider):
        with open('caulong.csv', 'a', encoding='utf-8', newline='') as csvfile:
            fieldnames = ['ma', 'tensp', 'gia', 'thuongHieu', 'courseUrl', 'tinhTrang', 'trinhDo', 'noiDung', 'phongCach', 'doCung', 'diemCanBang', 'weight_type', 'weight_range', 'thongTin']
